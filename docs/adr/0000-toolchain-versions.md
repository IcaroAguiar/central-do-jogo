...
| TypeScript | `6.0.3` | TypeScript 7.0 ships a native `tsc` but lacks a stable programmatic API; `typescript-eslint` and related tools remain blocked until ~7.1. |
| Vitest | `4.x` | Aligned with Vite 8 ecosystem. |
| ESLint | flat config + `typescript-eslint` 8.x | Current Vite React-TS convention. |
